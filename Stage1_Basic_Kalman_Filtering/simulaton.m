...
for rep = 1:N
    ...
    % Step 2: 模拟每个预测者观测带噪signal并进行Kalman信念更新
    signals = zeros(M,T-1);
    yhat = zeros(M,T-1);
    for i = 1:M
        for t = 2:T
...
for b = 1:B
    idx = randi(N, N, 1);                      % 有放回采样索引
    sample7 = stat_eq7(idx);
    sample8 = stat_eq8(idx);
    boot_stat_eq7(b) = mean(sample7);
    boot_stat_eq8(b) = mean(sample8);
end
...
%% 输出结果
fprintf('公式(7) 的均值统计量：%.4f\n', mean(stat_eq7));
fprintf('公式(8) 的均值统计量：%.4f\n', mean(stat_eq8));
fprintf('公式(7) 的Bootstrap拒绝区间：[%.3f, %.3f]\n', ci7_low, ci7_high);
fprintf('公式(8) 的Bootstrap拒绝区间：[%.3f, %.3f]\n', ci8_low, ci8_high);
fprintf('公式(7) 的power（拒绝率）：%.3f\n', power_eq7);
fprintf('公式(8) 的power（拒绝率）：%.3f\n', power_eq8);
